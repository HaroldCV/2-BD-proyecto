...
def load_data_in_postgres(docs_to_read):
    conn=None
    try:
        conn = psycopg2.connect("dbname=haroldcv user=haroldcv") # user and password may change depending on ur settings
...
def data_cargada(docs_to_read, c):
    if request.method == 'POST':
        if request.is_json:
            ...
            if query is None or topk is None:
                return jsonify({'error': 'Los campos de consulta (query) y topk son requeridos.'}), 400

            resultado, python_time =search(instance, int(docs_to_read), c, query, int(topk))

            postgres_resultado, postgres_time = postgres_search(query, topk)

            response_data = {
                'resultado': resultado,
                'postgres_resultado': postgres_resultado,
    ...
    return jsonify({'message': 'Top k hallado exitosamente'})
